...
float ADC1v = 0; //Lab 5: Define global vars to hold ADC1, 2 voltage
float ADC2v = 0;
...
void setupSpib(void) //Call this function in main() somewhere after the DINT; line of code.
{
    ...
    SpibRegs.SPITXBUF = 0x1300; //Lab 5: Write 00 to addr 13

    // To address 00x14 write 0x00
    // To address 00x15 write 0x00
    SpibRegs.SPITXBUF = 0x0000; //Lab 5: Write 00 to addr 14, 15

    // To address 00x16 write 0x00
    // To address 00x17 write 0x00
    SpibRegs.SPITXBUF = 0x0000; //Lab 5: Write 00 to addr 16, 17

    // To address 00x18 write 0x00
    // To address 00x19 write 0x13
    SpibRegs.SPITXBUF = 0x0013; //Lab 5: Write 00 to addr 18, 13 to addr 19

    // To address 00x1A write 0x02
    // To address 00x1B write 0x00
    SpibRegs.SPITXBUF = 0x0200; //Lab 5: Write 02 to addr 1A, 00 to addr 1B

    // To address 00x1C write 0x08
    // To address 00x1D write 0x06
    SpibRegs.SPITXBUF = 0x0806; //Lab 5: Write 08 to addr 1C, 06 to addr 1D

    // To address 00x1E write 0x00
    // To address 00x1F write 0x00
    SpibRegs.SPITXBUF = 0x0000; //Lab 5: Write 00 to addr 1E, 00 to addr 1F

    // wait for the correct number of 16-bit values to be received into the RX FIFO
    while (SpibRegs.SPIFFRX.bit.RXFFST != 7)
        ; //lab 5: we send 7 words, wait to recieve 7 back

    GpioDataRegs.GPCSET.bit.GPIO66 = 1; // Slave Select High
    temp = SpibRegs.SPIRXBUF;

    // ???? read the additional number of garbage receive values off the RX FIFO to clear out the RX FIFO
    temp = SpibRegs.SPIRXBUF; //Discard first read lAB 5 DISCARD extra 7 values
    ...
    SpibRegs.SPITXBUF = 0x2300; //lab 5 write 00 to register 23
    ...
    GpioDataRegs.GPCSET.bit.GPIO66 = 1; // Slave Select High
    temp = SpibRegs.SPIRXBUF;
    //lab 5 read the additional number of garbage receive values off the RX FIFO to clear out the RX FIFO
    temp = SpibRegs.SPIRXBUF;
    temp = SpibRegs.SPIRXBUF;
    temp = SpibRegs.SPIRXBUF;
    ...
    DELAY_US(10);
    GpioDataRegs.GPCCLEAR.bit.GPIO66 = 1;
    SpibRegs.SPITXBUF = (0x7700 | 0x00E9); // 0x7700 //lab 5 add additional offset of 255
    while (SpibRegs.SPIFFRX.bit.RXFFST != 1)
        ;
    GpioDataRegs.GPCSET.bit.GPIO66 = 1;
    temp = SpibRegs.SPIRXBUF;
...
void main(void)
{
    ...
    IER |= M_INT13;
    IER |= M_INT14;
    IER |= M_INT6; //lab 5  enable interrupt 6x
...
__interrupt void cpu_timer0_isr(void)
{
    ...
    SpibRegs.SPIFFRX.bit.RXFFIL = 3; // Issue the SPIB_RX_INT when three (lab 5) values are in the RX FIFO
    SpibRegs.SPITXBUF = 0x00DA; //Lab 5: Write 0x00DA to signal start of signal

    if (reverse)
    { //Lab 5: Either count up or down by 10
        pwmVal1 -= 10;
    }
    else
    {
    ...
    if (pwmVal1 == 3000)
    { //Lab 5: At the value 3000, start downcounting
        reverse = 1;
    }
    else if (pwmVal1 == 0)
    { //Lab 5: At the value 0, start upcounting
        reverse = 0;
    }
    ...
    GpioDataRegs.GPCCLEAR.bit.GPIO66 = 1;
    SpibRegs.SPIFFRX.bit.RXFFIL = 8; // Issue the SPIB_RX_INT when 8 (lab 5) values are in the RX FIFO

    SpibRegs.SPITXBUF = 0x3A00 | 0x8000 ;//Lab 5: request reg addr 3A, write 00 to get the value of 3A
    SpibRegs.SPITXBUF =  0x0000; //Lab 5:write 00 to get value of 3B,3C
    SpibRegs.SPITXBUF =  0x0000; //Lab 5: write 00 to get value of 3D, 3E
    SpibRegs.SPITXBUF =  0x0000; //Lab 5: write 00 to get value of 3F, 40
    SpibRegs.SPITXBUF =  0x0000; //Lab 5: write 00 to get value of 41, 42
    SpibRegs.SPITXBUF =  0x0000; //Lab 5: write 00 to get value of 43, 44
    SpibRegs.SPITXBUF =  0x0000; //Lab 5: write 00 to get value of 45, 46
    SpibRegs.SPITXBUF =  0x0000; //Lab 5: write 00 to get value of 47, 48
    //lab5 grabs the temp, accel, and gyro values and groups the high low values together
...
__interrupt void SPIB_isr(void)
{
    ...
    */
    ...
    RawAccelX = SpibRegs.SPIRXBUF; //Lab 5 Read in values for Accel
    RawAccelY = SpibRegs.SPIRXBUF; //Lab 5 Read in values for Accel
    RawAccelZ = SpibRegs.SPIRXBUF; //Lab 5 Read in values for Accel

    RawTemp = SpibRegs.SPIRXBUF; //Lab 5 read temp

    RawGyroX = SpibRegs.SPIRXBUF; //Lab 5 Read in values for Gyro
    RawGyroY = SpibRegs.SPIRXBUF; //Lab 5 Read in values for Gyro
    RawGyroZ = SpibRegs.SPIRXBUF; //Lab 5 Read in values for Gyro
