...
int32_t numTimer2calls = 0; //lab 2: initialize numTimer2calls
int16_t rowCounterTest = 0; //lab 2: testing counter
int16_t switchStateGlobal = 0; //lab2: switch state
...
void main(void)
{
    ...
    {
        if (UARTPrint == 1 ) {
			serial_printf(&SerialA,"Num Timer2:%ld Switch State:%d Num SerialRX: %ld\r\n",numTimer2calls,switchStateGlobal,numRXA); //lab 2: changed to print out numTimer2calls and switch state
            UARTPrint = 0;
            //SetLEDRowsOnOff(rowCounterTest); lab 2: used to test rows
        }
...
__interrupt void cpu_timer2_isr(void)
{
    numTimer2calls++; //lab 2: increment by 1 every time interrupt is called
    UARTPrint = 1; //lab 2: print every time interrupt is called
    rowCounterTest++; // lab2: for testing
	...
	switchStateGlobal = ReadSwitches(); //lab 2 : read switches
	SetLEDRowsOnOff(switchStateGlobal); // lab 2: set led rows based off of switches that are pressed.
}
